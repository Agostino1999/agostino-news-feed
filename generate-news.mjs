  ...
  for (let i = 0; i < 3; i++) {
    ...
    out = out
      ...
      .replace(/&([A-Za-z]+);/g, (m, n) =>
        Object.prototype.hasOwnProperty.call(named, n)
          ? named[n]
          : m
      );
  ...
  return items
    ...
    .map(item => {
      ...
      return {
        ...
        isVideo:
          /\b(video|filmato|telecamere|ripreso|riprese)\b/i.test(
            `${title} ${summary}`
          ),
    ...
    try {
      const res = await fetch(
        googleUrl(query),
        {
          headers: {
            "User-Agent":
              "Mozilla/5.0 (compatible; AgostinoNewsFeed/1.0)",
...
function finalClassify(a) {
  ...
  // Termini relativi a immigrazione / stranieri
  const immigration =
    /\b(immigrat|migrant|stranier|irregolar|clandestin|richiedent asil|profugh|sbarch|rimpatr|cpr|accoglienz|lampedusa|ong)\w*/.test(
      text
    );
  ...
  // Termini relativi a reati / sicurezza
  const crime =
    /\b(arrest|rapin|aggress|omicid|tentato omicidio|furto|violenz|stupro|stupr|molest|accoltell|coltell|pugni|picchi|rissa|spaccio|droga|evas|fuga|poliziott|carabinier|agente|denunciat|fermato)\w*/.test(
      text
    );

  // Solo se ci sono entrambi i segnali
  if (
    immigration &&
    crime
  ) {
...
async function main() {
  ...
  /*
   * Le query vengono fatte una alla volta,
   * non tutte contemporaneamente,
   * per ridurre il rischio di blocchi Google.
   */
  for (
    let i = 0;
    i < QUERIES.length;
  ...
  /*
   * Se Google produce improvvisamente
   * pochissime news,
   * manteniamo il feed precedente sano.
   */
  if (
    news.length <
      MIN_GOOD_ITEMS &&
